...
static void check_for_ignored_events(application &app)
{
...
static run_result run_load_rules_files(base_action &act)
{
...
std::shared_ptr<base_action> act_load_rules_files(application &app)
{
	std::list<std::string> prerequsites = {"load plugins"};

	return std::make_shared<action>("load rules files",
					"init",
					prerequsites,
					run_load_rules_files,
